...
export function ErrorState({ title, detail, onRetry }: { title: string; detail: string; onRetry: () => void }) {
  return <section className="panel error-state" role="alert"><h2>{title}</h2><p>{detail}</p><button className="button button-outline" onClick={onRetry}>Retry</button></section>;
}
