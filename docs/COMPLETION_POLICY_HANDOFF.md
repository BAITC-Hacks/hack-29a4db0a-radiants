...
The Backend integration calls `checkActivityCompletion` inside `completeActivity`'s
`.immediate()` transaction. It preserves 404/409 and uses the existing API code
**422 `EVENT_NOT_ELIGIBLE`** for domain refusals. Missing prerequisite levels appear
in `error.details`. History insertion, deterministic recomputation and audit commit
or roll back together. Stored assignment source/due date are preserved. HR cannot
complete another employee's activity. The assigned-mandatory and snapshot policies
below are adopted for the demo; the original HTTP regression is kept unchanged.

See [the active API handoff](FRONTEND_COMPLETION_GOAL_HANDOFF.md). Public forecast
fields are still awaiting agreement; the pure preview has not been exposed as a
new route or DTO. The verification report below is historical evidence from the
Domain/AI branch **before** Backend integration, not the final integrated result.

## Historical Domain/AI handoff (before Backend integration)
