...
**По умолчанию используются личные аккаунты.** При первой инициализации сервер создаёт:

| Логин | Доступ |
| --- | --- |
| `hr-admin` | HR: профили, аналитика, импорт и аккаунты |
| `employee` | Yerlan Kaliyev, профиль `E0178` |
| `employee2` | Ksenia Pavlova, профиль `E0058` |

У каждого аккаунта собственный случайный пароль. Оператор получает начальные данные входа локально:

```bash
docker compose exec app cat /app/.data/initial-access.json
```

Файл не отдаётся через HTTP и не включён в Git. Передавайте каждому сотруднику только его данные входа; при демонстрации не выводите весь файл на общий экран. Существующий volume сохраняет ранее созданные аккаунты и пароли.

### Демо-вход по имени и фамилии

Для показа синтетических профилей создайте `.env` в корне проекта на основе [.env.example](.env.example) и включите:

```dotenv
DEMO_EMPLOYEE_LOGIN=true
```

Примените настройку командой `docker compose up --build -d`. Теперь сотрудник входит так:

| Поле | Пример |
| --- | --- |
| Имя и фамилия | `Ksenia Pavlova` |
| Пароль | `admin` |
...
1. Войдите как `employee` с начальным личным паролем. Включённый демо-режим также позволяет открыть **Yerlan Kaliyev** с паролем `admin`.
2. Откройте цель **Backend Engineer → Senior**, навыки и историю. Для исходного `E0178` соответствие цели — **71,3%**.
